...
import { Close, StarBorder, Swipe} from '@mui/icons-material'
import {Swiper,SwiperSlide} from 'swiper/react'
import {Navigation,Autoplay,EffectCoverflow,Lazy,Zoom} from 'swiper/modules'
import 'swiper/css'
...
const Room = () => {
  ...
  useEffect(()=>{
    if(room){
      ...
      fetch(url).then(response=>response.json()).then(data=>setPlace(data.features[0]))
    }
  })
  const handleClose=()=>{
